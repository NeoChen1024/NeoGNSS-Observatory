...
void ObservationReader::finish() {
    ...
    if (!epochs_ && meas3_)
        throw std::runtime_error(
            "Meas3-only SBF is unsupported; enable MeasEpoch");
}
...
struct PppFloat::State {
    ...
    explicit State(const Json &s) : settings(s) {
        ...
        if (step_ns < 1 || gap_ns < 1)
            throw std::invalid_argument(
                "PPP interval below nanosecond resolution");
...
PppResult PppFloat::process(const ObservationBatch &batch) {
    ...
    for (const auto &e : batch.epochs) {
        ...
        for (const auto &m : e.signals) {
            ...
            if (m.phase_valid)
                s.tracks[key] = {e.gpst_ns, m.lock_seconds, m.lock_valid,
                                 m.half_cycle, m.sub_half_cycle};
            auto &o = rows[sat];
            ...
            if (o.code[slot])
                throw std::runtime_error(
                    "Duplicate selected observation in epoch");
        ...
        if (s.solved_time >= 0 &&
            e.gpst_ns - s.solved_time < s.step_ns - 1000000) {
            ++s.skipped;
            continue;
        }
        ...
        for (auto &[sat, o] : rows) {
            ...
            if (!nav.pcvs[sat - 1].type[0])
                throw std::runtime_error(
                    "Missing satellite antenna calibration");
            for (int k = 0; k < 2; ++k)
                o.P[k] -= s.bias(sat, k ? s.code2 : s.code1, e.gpst_ns);
            // No extrapolation beyond actual product coverage or SP3-clock
            // fallback.
            ...
            auto upper = std::lower_bound(nav.pclk, nav.pclk + nav.nc, transmit,
                                          [](const pclk_t &a, gtime_t t) {
                                              return timediff(a.time, t) < 0;
                                          });
            if (upper == nav.pclk || upper == nav.pclk + nav.nc ||
                timediff(upper->time, (upper - 1)->time) > 60 ||
                upper->clk[sat - 1][0] == 0 ||
                (upper - 1)->clk[sat - 1][0] == 0)
                throw std::runtime_error("Missing bracketing 30-second CLK "
                                         "samples; no SP3-clock fallback");
        ...
        // Pinned PPP layout: static XYZ, NSYS clocks, estimated ZTD (nf=2,
        // IFLC).
