...
struct Batch {
  ...
  py::tuple export_array(py::object requested) {
    ...
    auto s = new ArrowSchema(schema);
    schema.release = nullptr;
    auto a = new ArrowArray(array);
    array.release = nullptr;
    py::capsule sc(s, "arrow_schema", [](PyObject *c) {
      auto p =
    ...
    py::capsule ac(a, "arrow_array", [](PyObject *c) {
      auto p =
...
std::shared_ptr<Batch> observations() {
  ...
  const char *names[] = {"pseudorange_m", "carrier_phase_cycles", "doppler_hz",
                         "cn0_db_hz"};
  const char *quality[] = {"code_quality", "phase_quality", "doppler_quality"};
  for (int i = 0; i < 4; ++i)
...
void append(Batch &b, const cppgnss::Measurement &m, Tick t,
  ...
  if (m.lock_ms) {
    ...
    str(l->children[2], m.lock_lower_bound ? "lower_bound" : "reported_value");
...
struct Reader {
  ...
  Reader(const std::string &protocol, const std::string &id, unsigned ant)
    ...
    if (protocol == "ubx" && ant != 0)
      throw std::invalid_argument("RAWX importer supports antenna 0 only");
  }
  ...
  py::class_<Reader>(m, "CnexObservationReader")
      ...
      .def("feed",
           [](Reader &r, py::bytes bytes) {
             ...
             return r.feed({reinterpret_cast<const uint8_t *>(p), size_t(n)});
           })
