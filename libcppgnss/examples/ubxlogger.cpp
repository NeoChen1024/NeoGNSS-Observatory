...
static ByteReadResult tcp_read_byte(void *context) {
    ...
    while (true) {
        ...
        if (n == 1)
            return {.result = ReadResult::ok, .byte = c};
        if (n == 0)
            return {.result = ReadResult::end};
        if (errno == EINTR)
            continue;
...
static ReadResult read_logged_frame(void *context, read_byte_fn read_byte,
                                    ubx_buf_t &buf) {
    size_t discarded = 0;
...
static void log_parse_error(std::string_view detail,
                            std::source_location location) {
    auto message =
        std::format("{}:{} {}: {}\n", location.file_name(), location.line(),
                    location.function_name(), detail);
...
void print_status_line(const ubx_nav_pvt &pvt) {
    ...
    fputs(buf, stderr);
    auto status =
        std::format("\riTOW={:06}.{:03} GPST {:>10}, Sats: {:02}",
...
static void print_stats(Stats &st, time_t now) {
    ...
    if (st.period_pvt > 0) {
        ...
        auto message = std::format("\n[neoubxlogger stats] avg rate: {:.1f} "
                                   "KiB/s, frames: {}, FIX {}%\n",
...
int main(int argc, char *argv[]) try {
    ...
    while ((opt = getopt_long(argc, argv, "f:t:dnq", long_options, nullptr)) !=
           -1) {
        switch (opt) {
        case 1000:
        case 1001: {
            ...
            if (parsed.ec != std::errc{} || parsed.ptr != end ||
                value < (opt == 1000 ? 1 : 0) ||
                value >= (opt == 1000 ? 604800000 : 100)) {
                fputs("Invalid expected period (1..604799999 ms) or tolerance "
                      "(0..99 percent)\n",
                      stderr);
                RETURN_ERR;
            }
            if (opt == 1000)
                diagnostics.interval_ms = value;
            else
                diagnostics.tolerance_percent = value;
            break;
        }
        case 1002:
        ...
        case 't': {
            ...
            if (error != std::errc{} || end != port_end || tcp_port < 1 ||
                tcp_port > 65535) {
            ...
            if (tcp_host.front() == '[') {
                if (tcp_host.size() <= 2 || tcp_host.back() != ']') {
                    fprintf(stderr,
                            "Invalid TCP spec '%s'. Expected HOST:PORT\n",
                            optarg);
                    RETURN_ERR;
                }
        ...
        default:
            fprintf(stderr,
                    "Usage: %s [-f input_file] [-t HOST:PORT] [-n] [-d] [-q] "
                    "[--expected-period-ms 1000] [--epoch-tolerance-percent "
                    "20] [--expect-nav-clock] [OUTPUT_DIR]\n",
                    argv[0]);
            RETURN_ERR;
        }
    }
    ...
    if (argc - optind > 1) {
        fprintf(stderr,
                "Unexpected argument: %s (only one OUTPUT_DIR is allowed)\n",
                argv[optind + 1]);
        RETURN_ERR;
    }
    if (optind < argc)
        output_root = argv[optind];
    if (input_path) {
    ...
    while (1) {
        ...
        if (tcp_mode) {
            ...
            if (ret == ReadResult::timeout) {
                ...
                fprintf(stderr,
                        "\nTCP %s:%d: read timeout; discarding partial frame "
                        "and resynchronizing\n",
                        tcp_host.c_str(), tcp_port);
                continue;
            }
        ...
        if (frame.class_id == 1 && frame.msg_id == 0x22 &&
        ...
        if (frame.class_id == 1 && frame.msg_id == 0x20) {
            ...
            const int64_t value =
                int64_t(read_le<int16_t>(p, 8)) * 604800000 + tow;
            if (have_timegps && value != epoch_gpst_ms) {
                const auto detail = std::format(
                    "[logger qc] missing_EOE_or_conflicting_TIMEGPS "
                    "previous={} incoming={}\n",
                    LoggerDiagnostics::label(epoch_gpst_ms),
                    LoggerDiagnostics::label(value));
        ...
        if (ubx_nav_eoe_semantically_valid(eoe)) {
            if (!have_timegps || epoch_tow != eoe.data.iTOW) {
                fprintf(stderr,
                        "[logger qc] invalid_epoch have_TIMEGPS=%d "
                        "TIMEGPS_iTOW_ms=%u EOE_iTOW_ms=%u\n",
            ...
            if (epoch_gpst_ms <= last_gpst_ms) {
                const auto detail =
                    std::format("[logger qc] non_increasing_epoch previous={} "
                                "incoming={}\n",
                                LoggerDiagnostics::label(last_gpst_ms),
                                LoggerDiagnostics::label(epoch_gpst_ms));
                fputs(detail.c_str(), stderr);
                fputs("Non-increasing GPST epoch\n", stderr);
                RETURN_ERR;
            }
            diagnostics.epoch(epoch_gpst_ms, epoch_clock_count,
                              epoch_clock_tow);
            epoch_clock_count = 0;
            ...
            if (!no_write && day != output_day) {
                ...
                const auto calendar =
                    sys_days{year{1980} / 1 / 6} + milliseconds{epoch_gpst_ms};
                const auto dirname =
                    output_root / std::format("{:%Y-%m}", calendar);
                ...
                const auto filename =
                    ...
                    std::format("GPST-{:%Y-%m-%d--%H-%M-%S}-{:03}.ubx",
                                floor<seconds>(calendar), epoch_gpst_ms % 1000);
                if (!output.open(filename.c_str()))
                    RETURN_ERR;
                output_day = day;
                if (!quiet)
                    fprintf(stderr, "\nOpened file %s\n", filename.c_str());
            }
            if (!no_write && fwrite(pending.data(), 1, pending.size(),
                                    output.get()) != pending.size()) {
                perror("UBX output");
                RETURN_ERR;
            }
            pending.clear();
    ...
    if (!pending.empty() || have_timegps) {
        fputs("Incomplete epoch at EOF: missing EOE; pending bytes not "
              "published\n",
              stderr);
        RETURN_ERR;
    }
    ...
    fputs("Logger error: unknown exception\n", stderr);
    return EXIT_FAILURE;
}
