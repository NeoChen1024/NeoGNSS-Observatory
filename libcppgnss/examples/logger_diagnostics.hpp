...
struct LoggerDiagnostics {
    ...
    static std::string label(int64_t ms) {
        ...
        return std::format("GPST-{:%Y-%m-%d--%H-%M-%S}-{:03}",
                           floor<seconds>(date), ms % 1000);
    }
    ...
    void epoch(int64_t ms, uint32_t clock_count, uint32_t clock_tow) {
        ...
        if (previous_ms >= 0) {
            ...
            if (std::abs(dt - interval_ms) * 100 >
                interval_ms * tolerance_percent) {
                const auto arrival =
                    std::chrono::duration_cast<std::chrono::milliseconds>(
                        now - previous_arrival)
                        .count();
                const auto slots = std::llround(double(dt) / interval_ms);
                const bool aligned =
                    slots >= 1 && std::abs(dt - slots * interval_ms) * 100 <=
                                      interval_ms * tolerance_percent;
                const bool gap =
                    dt * 100 > interval_ms * (100 + tolerance_percent);
                if (gap)
                    ++gaps;
                if (!gap || !aligned)
                    ++irregular;
        ...
        if (clock_count && clock_tow % 604800000 != ms % 604800000) {
            ++mismatched_clock;
            const auto text = std::format("\n[logger qc] mismatched_NAV-CLOCK "
                                          "epoch={} clock_iTOW_ms={}\n",
                                          label(ms), clock_tow);
            fputs(text.c_str(), stderr);
        }
