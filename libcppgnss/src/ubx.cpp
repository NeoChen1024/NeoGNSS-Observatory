...
static thread_local parse_error_handler error_handler = nullptr;

parse_error_handler set_parse_error_handler(parse_error_handler handler) {
    auto previous = error_handler;
    error_handler = handler;
    return previous;
}

void report_parse_error(std::string_view detail,
                        std::source_location location) {
    if (error_handler)
        error_handler(detail, location);
}
...
ubx_frame::ubx_frame(std::span<const uint8_t> buf) {
    ...
    this->payload =
        ubx_buf_t(buf.begin() + UBX_HEADER_SIZE, buf.end() - UBX_CKSUM_SIZE);
}
...
bool ubx_frame::validate(std::span<const uint8_t> buf) {
    ...
    if (buf.size() != (size_t)this->length + UBX_HEADER_SIZE + UBX_CKSUM_SIZE) {
        report_parse_error(
            std::format("buffer size {} does not match payload length {}",
...
int ubx_frame::write(FILE *fp) const {
    if (fputc(UBX_SYNC1, fp) == EOF || fputc(UBX_SYNC2, fp) == EOF ||
        fputc(this->class_id, fp) == EOF || fputc(this->msg_id, fp) == EOF ||
...
ubx_any_msg::ubx_any_msg() { clear(); }

ubx_any_msg::ubx_any_msg(const ubx_frame &frame) { parse(frame); }
