...
def reversal_message(error, segments, *, raw_bits=False):
    offset = error["offset"]
    ...
    return (
        ...
        + (
            "Preserving RawBits timestamp and payload unchanged; SIS timestamps need not follow output order."
            if raw_bits
            else "Possible overlap or timestamp disorder; inspect/re-stitch the inputs before importing."
        )
    )
...
def run(inputs, station, protocol, rebuild, resume_from, chunk_mib, source_antenna, recursive):
    ...
    try:
        ...
        def check_tail(key, batch):
            if mode != "tail" or key[1] == "raw-bits":
                return
            ...
            for path, start, size in segments:
                with path.open("rb") as source:
                    ...
                    while remaining:
                        ...
                        reversals = summary["raw_bits_time_reversals"]
                        if reversals >= raw_warning_threshold:
                            tqdm.write(
                                f"Warning: {reversals} RawBits time reversals so far; latest: "
                                + reversal_message(summary["last_raw_bits_time_reversal"], segments, raw_bits=True),
                                file=sys.stderr,
                            )
                            raw_warning_threshold = 1 << reversals.bit_length()
